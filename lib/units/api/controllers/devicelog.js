...
function getLogs(req, res) {
  ...
  //  console.log("getLogs page:" + page + ",count:" + count)
    ...
    if (field == '' && filter == '') {
        return dbapi.getLogsCount(startdate, enddate)
            .then(function(ret) {
              //  console.log("Recv ret:" + JSON.stringify(ret));
                var countall = ret

                return dbapi.getLogs(startdate, enddate, page, count)
        ...
        return dbapi.getLogsFilterCount(startdate, enddate, field, filter)
            .then(function(ret) {
              //  console.log("Recv ret:" + JSON.stringify(ret));
                var countall = ret
                return dbapi.getLogsFilter(startdate, enddate, field, filter, page, count)
                    .then(function(cursor) {
                        return Promise.promisify(cursor.toArray, cursor)()
                            .then(function(list) {
                                var datas = []
...
function getStatisticsPerCustom(req, res) {
  ...
  //  console.log("end:" + enddate)
    ...
    arrays.forEach(value => {
      //  console.log("group by item:"+value)
    ...
    return dbapi.getStatticsCountPerCustom(startdate, enddate, group_by)
        .then(function(ret) {
          //  console.log("Recv Rows cnt:" + JSON.stringify(ret));
            var countall = ret
            return dbapi.getStatticsPerCustom(startdate, enddate, group_by, page, count, order_by, asc_desc)
                ...
                .catch(function(err) {
                    log.error('[Error]Failed to getStatticsPerCustom: ', err.stack)
                    res.status(500).json({
                        success: false
                    })
                })
        }).catch(function(err) {
            log.error('[Error]Failed to getStatticsCountPerCustom: ', err.stack)
            res.status(500).json({
                success: false
            })
        })
